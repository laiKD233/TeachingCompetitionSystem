  ...
  <div class="dashboard-page">
    ...
    <div class="charts-section">
      ...
      <el-card class="chart-card">
        ...
        <div class="chart-container">
          <div ref="competitionStatsChartRef" style="width: 100%; height: 300px;"></div>
        </div>
      </el-card>
    </div>
...
import { getDashboardStats, getUserTrend, getCompetitionStats } from '@/api/super-admin'
...
const competitionStatsChartRef = ref(null)
...
onUnmounted(() => {
  ...
  if (competitionStatsChart) competitionStatsChart.dispose()
})
...
const fetchChartData = async () => {
  ...
  try {
    ...
    const days = daysMap[userTrendPeriod.value] || 7
    ...
    if (trendRes.status === 'fulfilled' && trendRes.value.data) {
      renderUserTrendChart(trendRes.value.data)
    }

    if (statsRes.status === 'fulfilled' && statsRes.value.data) {
      renderCompetitionStatsChart(statsRes.value.data)
    }
  } catch (error) {
...
const renderCompetitionStatsChart = (data) => {
  if (!competitionStatsChartRef.value) return
  if (competitionStatsChart) competitionStatsChart.dispose()
  competitionStatsChart = echarts.init(competitionStatsChartRef.value)
  const chartData = data.typeData && data.typeData.length > 0 ? data.typeData : data.statusData
  competitionStatsChart.setOption({
