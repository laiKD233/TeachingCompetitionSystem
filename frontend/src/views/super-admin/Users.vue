<template>
  <div class="users-page">
        ...
        <el-table-column label="操作" fixed="right" width="240">
          <template #default="scope">
            ...
            <el-button
              ...
              link
              size="small"
              @click="handleAssignCompetitions(scope.row)"
            >
              分配竞赛
            </el-button>
          </template>
    ...
    >
      <div class="assign-info" v-if="currentUser">
        <p>为管理员 <strong>{{ currentUser.name }}</strong> 分配管理的竞赛</p>
        <p class="assign-tip">提示：不分配竞赛则默认管理所有竞赛</p>
      </div>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="handleSubmitAssign">确定分配</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import {
  ...
  assignCompetition, removeCompetition, getAdminCompetitions, getAllCompetitions
...
const handleAssignCompetitions = async (row) => {
  currentUser.value = row
  selectedCompetitions.value = []
  assignDialogVisible.value = true

  try {
    const [competitionsRes, adminCompetitionsRes] = await Promise.all([
      getAllCompetitions(),
      getAdminCompetitions(row.id)
    ])

    allCompetitions.value = competitionsRes.data || []

    const assignedIds = (adminCompetitionsRes.data || []).map(c => c.id)
    selectedCompetitions.value = assignedIds
  } catch (error) {
...
const handleSubmitAssign = async () => {
  ...
  try {
    const adminId = currentUser.value.id
