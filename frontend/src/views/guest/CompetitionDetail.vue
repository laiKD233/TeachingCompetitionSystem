  ...
  <div class="competition-detail-page" v-loading="loading">
    ...
    <div class="detail-body" v-if="competition">
      ...
      <div class="info-section">
        ...
        <div class="info-grid">
          ...
          <div class="info-item">
            <span class="info-label">参赛方式</span>
            <span class="info-value">
              <el-tag :type="competition.participationType === 'TEAM' ? 'warning' : 'success'" effect="plain" size="small">
                {{ competition.participationType === 'TEAM' ? '团队赛' : '个人赛' }}
              </el-tag>
            </span>
          </div>
          <div class="info-item">
      ...
      <div class="action-section" v-if="competition.status === 'REGISTRATION'">
        ...
        <template v-else-if="!isLoggedIn">
          <el-button type="primary" size="large" @click="handleLoginFirst" class="cta-button">
            <el-icon><User /></el-icon>
            登录后报名
          </el-button>
          <p class="login-tip">请先登录系统后再报名参赛</p>
        </template>
        <template v-else>
      ...
      <!-- 获奖公示信息（公示中或已结束时显示） -->
      <div v-if="isCompetitionResultVisible(competition.status)" class="results-section">
        <h3 class="section-title">
        ...
        <template v-else-if="awardResults.length > 0">
          <div class="winner-list">
            <div v-for="item in awardResults" :key="item.workId" class="winner-card">
              ...
              <div class="winner-info">
                <div class="winner-name">{{ getWinnerName(item) }}</div>
          ...
          <el-card v-if="allResults.length > 0" class="full-results-card">
            ...
            <el-table :data="allResults" stripe size="small">
              ...
              <el-table-column label="参赛对象" width="120">
                <template #default="scope">
                  <span>{{ getWinnerName(scope.row) }}</span>
                </template>
              </el-table-column>
    ...
    >
      <el-form
        ...
        class="register-form"
      >
        <template v-if="!(competition && competition.participationType === 'TEAM' && registerForm.teamAction === 'join')">
          <el-form-item label="项目名称" prop="projectName">
            <el-input v-model="registerForm.projectName" placeholder="请输入项目名称" />
          </el-form-item>
          <el-form-item label="参赛赛道" v-if="tracks.length > 0">
            <el-select v-model="registerForm.trackId" placeholder="请选择参赛赛道">
              <el-option label="请选择赛道" value="" />
              <el-option v-for="track in tracks" :key="track.id" :label="track.name" :value="track.id" />
            </el-select>
          </el-form-item>
          <el-form-item label="指导教师">
            <el-select
              v-model="registerForm.advisorId"
              placeholder="请选择系统中的指导教师（可选）"
              clearable
              filterable
              style="width: 100%"
            >
              <el-option
                ...
                :key="advisor.id"
                :label="formatAdvisorLabel(advisor)"
                :value="advisor.id"
              />
            </el-select>
          </el-form-item>
        ...
        <template v-if="competition && competition.participationType === 'TEAM'">
          <el-divider content-position="left">团队信息</el-divider>
          <el-form-item label="团队选择" prop="teamAction">
            <el-radio-group v-model="registerForm.teamAction">
              <el-radio label="create">创建团队</el-radio>
              <el-radio label="join">加入团队</el-radio>
            </el-radio-group>
          </el-form-item>

          <template v-if="registerForm.teamAction === 'create'">
            ...
            <el-form-item label="团队描述">
              <el-input
                type="textarea"
                v-model="registerForm.teamDescription"
                :rows="2"
          ...
          <template v-if="registerForm.teamAction === 'join'">
            <el-alert type="info" :closable="false" show-icon style="margin-bottom: 16px;">
              加入团队后，项目名称等信息由团长填写，您无需重复填写。
            </el-alert>
            <el-form-item label="团队ID" prop="teamId">
              <el-input v-model="registerForm.teamId" placeholder="请输入要加入的团队ID" />
            </el-form-item>
            <el-form-item label="邀请码" prop="inviteCode">
              <el-input v-model="registerForm.inviteCode" placeholder="请输入团队邀请码" />
            </el-form-item>
          </template>
        </template>
      </el-form>
...
import { getAdminCompetition, getCompetition } from '@/api/competition'
import { createRegistration } from '@/api/registration'
import { getPublicResults } from '@/api/award'
import { createTeam, joinTeam, getAvailableAdvisors } from '@/api/team'
import { getTracksByCompetitionId } from '@/api/track'
import { useUserStore } from '@/stores/user'
import { getCompetitionStatusText, getCompetitionStatusType, isCompetitionResultVisible } from '@/utils/competitionStatus'
...
const isAdminPreview = computed(() => {
  ...
  return route.query.admin === '1' && ['ADMIN', 'TEACHER', 'ADVISOR'].includes(role)
})
...
const fetchCompetition = async () => {
  loading.value = true
  try {
    const res = isAdminPreview.value
      ? await getAdminCompetition(route.params.id)
      : await getCompetition(route.params.id)
    competition.value = res.data
    // 如果竞赛已公示或已结束，加载获奖信息
    if (competition.value && isCompetitionResultVisible(competition.value.status)) {
      fetchResults()
    }
    // 加载赛道列表
    await fetchTracks()
...
const fetchTracks = async () => {
  try {
    const res = await getTracksByCompetitionId(route.params.id)
...
const fetchAdvisors = async () => {
  try {
    ...
    advisors.value = (res.data || []).filter(item => ['TEACHER', 'ADVISOR'].includes(item.role))
  } catch (error) {
    console.error('获取指导教师列表失败', error)
    advisors.value = []
  }
}
...
const handleRegister = async () => {
  ...
  try {
    const advisorId = registerForm.value.advisorId ? Number(registerForm.value.advisorId) : null
    const selectedAdvisor = advisors.value.find(item => item.id === advisorId)
    const baseData = {
      competitionId: route.params.id,
      participationType: competition.value.participationType || 'INDIVIDUAL',
      projectName: registerForm.value.projectName,
      advisorId,
      advisor: selectedAdvisor?.name || '',
      description: registerForm.value.description,
      trackId: registerForm.value.trackId ? Number(registerForm.value.trackId) : null
    }

    if (competition.value.participationType === 'TEAM') {
      if (registerForm.value.teamAction === 'create') {
        if (!registerForm.value.teamName) {
          ElMessage.warning('请输入团队名称')
          submitting.value = false
          return
        }
        if (!registerForm.value.projectName) {
          ElMessage.warning('请输入项目名称')
          submitting.value = false
          return
        }
        const teamRes = await createTeam({
          name: registerForm.value.teamName,
          description: registerForm.value.teamDescription,
          competitionId: route.params.id,
        ...
        ElMessage.success(`报名成功！已创建团队，团队ID: ${team.id}，邀请码: ${team.inviteCode}`)
      } else {
        if (!registerForm.value.teamId || !registerForm.value.inviteCode) {
          ElMessage.warning('请输入团队ID和邀请码')
          submitting.value = false
          return
        }
        await joinTeam(registerForm.value.teamId, registerForm.value.inviteCode)
        await createRegistration({
          competitionId: route.params.id,
          participationType: 'TEAM',
          teamId: Number(registerForm.value.teamId),
          trackId: registerForm.value.trackId ? Number(registerForm.value.trackId) : null
        })
        ElMessage.success('报名成功！已加入团队')
      }
    } else {
      if (!registerForm.value.projectName) {
        ElMessage.warning('请输入项目名称')
...
const getStatusType = (status) => {
    return getCompetitionStatusType(status)
}
...
const getWinnerName = (item) => {
  if (!item) return '-'
  if (item.participationType === 'TEAM') {
    return item.teamName || item.participantName || '-'
  }
  return item.participantName || '-'
}

const formatAdvisorLabel = (advisor) => {
  if (!advisor) return '-'
  const roleText = advisor.role === 'TEACHER' ? '竞赛管理员' : '指导老师'
  return `${advisor.name || advisor.username}（${roleText}${advisor.college ? ' · ' + advisor.college : ''}）`
}
</script>
