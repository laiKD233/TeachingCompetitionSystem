<template>
  <el-container class="layout-container">
      ...
      <div class="sidebar-body">
        ...
        >
          ...
          <div class="menu-section" v-if="userStore.user?.role !== 'GUEST'">
            ...
            <el-menu-item :index="getDashboardPath()">
          ...
          <template v-if="userStore.user?.role === 'STUDENT'">
            ...
            <div class="menu-section">
              <span class="menu-section-label" v-show="!isCollapse">团队协作</span>
              <el-menu-item index="/participant/teams">
                <el-icon><UserFilled /></el-icon>
                <template #title><span>我的团队</span></template>
              </el-menu-item>
              <el-menu-item index="/participant/todos">
                <el-icon><List /></el-icon>
                <template #title><span>个人待办</span></template>
              </el-menu-item>
              <el-menu-item index="/participant/appointments">
                <el-icon><Calendar /></el-icon>
                <template #title><span>预约指导</span></template>
              </el-menu-item>
            </div>
          </template>
          ...
          <template v-if="userStore.user?.role === 'TEACHER' || userStore.user?.role === 'ADMIN'">
            ...
            <div class="menu-section">
              <span class="menu-section-label" v-show="!isCollapse">团队指导</span>
              <el-menu-item index="/participant/teams">
                <el-icon><UserFilled /></el-icon>
                <template #title><span>团队列表</span></template>
              </el-menu-item>
              <el-menu-item index="/admin/team-guide">
                <el-icon><UserFilled /></el-icon>
                <template #title><span>团队指导</span></template>
              </el-menu-item>
              <el-menu-item index="/admin/schedule">
                <el-icon><Calendar /></el-icon>
                <template #title><span>日程安排</span></template>
              </el-menu-item>
              <el-menu-item index="/admin/appointment-manage">
          ...
          <!-- 指导老师菜单 -->
          <template v-if="userStore.user?.role === 'ADVISOR'">
            <div class="menu-section">
              <span class="menu-section-label" v-show="!isCollapse">评审工作</span>
              <el-menu-item index="/admin/my-reviews">
                <el-icon><EditPen /></el-icon>
                <template #title><span>我的评审</span></template>
              </el-menu-item>
              <el-menu-item index="/admin/registrations">
                <el-icon><Document /></el-icon>
                <template #title><span>报名查看</span></template>
              </el-menu-item>
              <el-menu-item index="/admin/results">
                <el-icon><Trophy /></el-icon>
                <template #title><span>成绩公示</span></template>
    ...
    <Teleport to="body">
      <div v-if="showNotifications" class="notification-mask" @click="showNotifications = false"></div>
      <div v-if="showNotifications" class="notification-panel" @click.stop>
        <div class="notification-header">
          <span class="notification-title">通知</span>
          <el-button v-if="notificationCount > 0" type="primary" link size="small" @click="handleMarkAllRead">全部已读</el-button>
        </div>
        <div class="notification-list">
          <div v-if="notificationList.length === 0" class="notification-empty">暂无通知</div>
          <div
...
import { getUnreadCount, getMyNotifications, markAsRead, markAllAsRead } from '@/api/notification'
...
const breadcrumbs = computed(() => {
  ...
  const routeMap = {
    ...
    '/participant/teams': { title: '我的团队', path: '/participant/teams' },
    '/participant/todos': { title: '个人待办', path: '/participant/todos' },
    '/participant/appointments': { title: '预约指导', path: '/participant/appointments' },
    '/admin/dashboard': { title: userStore.user?.role === 'ADVISOR' ? '指导首页' : '管理首页', path: '/admin/dashboard' },
    ...
    '/admin/team-guide': { title: '团队指导', path: '/admin/team-guide' },
    '/admin/schedule': { title: '日程安排', path: '/admin/schedule' },
    '/admin/appointment-manage': { title: '预约管理', path: '/admin/appointment-manage' },
  ...
  // 处理团队详情页
  if (path.startsWith('/participant/team/')) {
    routes.push({ title: '团队详情', path: path })
  }
  return routes
})
...
const getDashboardPath = () => {
  ...
  if (role === 'STUDENT') return '/participant/dashboard'
  if (role === 'TEACHER' || role === 'ADMIN' || role === 'ADVISOR') return '/admin/dashboard'
  return '/guest/home'
}

const getDashboardTitle = () => {
  const role = userStore.user?.role
  if (role === 'STUDENT') return '我的首页'
  if (role === 'TEACHER' || role === 'ADMIN') return '管理首页'
  if (role === 'ADVISOR') return '指导首页'
  return '首页'
}
