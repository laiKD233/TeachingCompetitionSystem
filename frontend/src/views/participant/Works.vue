  ...
  <div class="works-page">
    ...
    <div class="works-stats">
      ...
      <div class="stat-chip chip-pending">
        ...
        <span class="stat-chip-value">{{ works.filter(w => ['PENDING', 'SUBMITTED'].includes(w.status)).length }}</span>
        <span class="stat-chip-label">已提交</span>
      </div>
          ...
          <div class="card-body">
            ...
            <div class="card-actions">
              ...
              <el-button
                ...
                size="small"
                :disabled="isWorkLocked(item)"
                :title="getWorkLockReason(item)"
    ...
    >
      <el-form :model="uploadForm" :rules="uploadRules" ref="uploadFormRef" label-position="top">
        <el-form-item label="竞赛" prop="competitionId">
          <el-select
            v-model="uploadForm.competitionId"
            placeholder="请选择竞赛"
            style="width: 100%"
            :disabled="uploadForm.isEdit || uploadForm.isReupload"
          >
            <el-option
        ...
        <el-form-item label="作品文件" prop="file">
          <div v-if="currentFileName && !existingFileRemoved && !selectedFile" class="current-file-box">
            <div class="current-file-info">
              <el-icon><Document /></el-icon>
              <span>{{ currentFileName }}</span>
            </div>
            <el-button type="danger" text @click="handleRemoveExistingFile">
              删除当前文件
            </el-button>
          </div>
          <el-alert
            v-if="existingFileRemoved && !selectedFile"
            type="warning"
            :closable="false"
            show-icon
            class="remove-file-alert"
            title="当前文件将在提交后被删除。如需替换文件，请在下方重新选择。"
          />
...
import { downloadWorkFile } from '@/utils/fileDownload'
...
const fetchCompetitionOptions = async () => {
  try {
    ...
    records.forEach(item => {
      if (item.competitionId && isSubmissionOpen(item) && !competitionMap.has(item.competitionId)) {
...
const handleOpenUpload = () => {
  if (competitionOptions.value.length === 0) {
    ElMessage.warning('当前没有可提交作品的已通过报名')
    return
  }
...
const handleEdit = (row) => {
  if (isWorkLocked(row)) {
    ElMessage.warning(getWorkLockReason(row))
    return
  }
    ...
    if (valid) {
      ...
      try {
        if (uploadForm.isEdit) {
          ...
          formData.append('removeFile', existingFileRemoved.value ? 'true' : 'false')
        ...
        ElMessage.error(error.message || ((uploadForm.isEdit || uploadForm.isReupload) ? '更新失败' : '上传失败'))
        console.error(error)
      } finally {
        uploading.value = false
      }
...
const handleDownload = async (row) => {
  ...
  try {
    await downloadWorkFile(row.fileUrl, row.fileName || row.title || '作品文件')
  } catch (error) {
...
const isSubmissionOpen = (item) => {
  const status = normalizeStatus(item.competitionStatus || item.status)
  if (['DRAFT', 'REVIEW', 'ANNOUNCEMENT', 'FINISHED', 'ENDED'].includes(status)) return false
  if (isPast(item.submissionDeadline)) return false
  return ['PUBLISHED', 'REGISTRATION', 'SUBMISSION'].includes(status) || !status
}

const getWorkLockReason = (item) => {
  if (item.avgScore !== null && item.avgScore !== undefined) return '作品已有成绩，不能修改或删除'
  const status = normalizeStatus(item.competitionStatus)
  if (['REVIEW', 'ANNOUNCEMENT', 'FINISHED', 'ENDED'].includes(status)) {
    return '竞赛已进入评审、公示或结束阶段，不能修改或删除作品'
  }
  if (isPast(item.submissionDeadline)) return '作品提交截止时间已结束，不能修改或删除'
  return ''
}

const isWorkLocked = (item) => Boolean(getWorkLockReason(item))
