  ...
  <div class="registrations-page">
    ...
    <div class="page-header-wrapper">
      ...
      <div class="page-header">
        ...
        <el-button type="primary" size="large" class="action-btn" @click="router.push('/participant/competitions')">
    ...
    <el-card class="table-card">
      ...
      <el-table :data="registrations" v-loading="loading" stripe>
        <el-table-column prop="competitionName" label="竞赛名称" min-width="180">
          <template #default="scope">
            <span
              ...
              @click="router.push(`/competition/${scope.row.competitionId}`)"
            >
        ...
        <el-table-column prop="teamName" label="团队" width="130">
          <template #default="scope">
            <span v-if="scope.row.teamName" class="team-link" @click="goToTeam(scope.row.teamId)">{{ scope.row.teamName }}</span>
        ...
        <el-table-column label="操作" fixed="right" width="190">
          <template #default="scope">
            ...
            <el-button
              v-if="canDeleteRegistration(scope.row)"
              type="danger"
              link
              size="small"
              class="action-link"
...
import { getRegistrations, deleteRegistration } from '@/api/registration'
...
const goToTeam = (teamId) => {
  if (teamId) {
    router.push(`/participant/team/${teamId}`)
  }
}

const canDeleteRegistration = (row) => {
...
const handleDeleteRegistration = async (row) => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定删除“${row.projectName || row.competitionName || '该报名'}”吗？删除后不可恢复。${teamTip}`,
      '删除报名',
      {
