...
import { isCompetitionResultVisible } from '@/utils/competitionStatus'
...
const fetchCompetitions = async () => {
  try {
    const [registrationRes, competitionRes] = await Promise.all([
      ...
      getPublicCompetitions({ page: 1, size: 100 })
    ])

    const visibleCompetitionMap = new Map(
      (competitionRes.data?.records || [])
        .filter(item => isCompetitionResultVisible(item.status))
        .map(item => [item.id, item])
    )
    const seen = new Set()
    competitionList.value = (registrationRes.data?.records || [])
      .filter(item => item.status === 'APPROVED')
      .filter(item => visibleCompetitionMap.has(item.competitionId))
      .filter(item => {
        if (seen.has(item.competitionId)) return false
        seen.add(item.competitionId)
        return true
      })
      .map(item => ({
        id: item.competitionId,
        name: item.competitionName || visibleCompetitionMap.get(item.competitionId)?.name || '未命名竞赛'
      }))
  } catch (error) {
