  ...
  <div class="competitions-page">
    ...
    <div class="page-header-wrapper">
      ...
      <div class="page-header">
        <div class="header-content">
          ...
          <p class="page-subtitle">创建、编辑和管理所有竞赛</p>
        </div>
        <el-button type="primary" size="large" class="action-btn" @click="handleCreate">
    ...
    <el-card class="table-card">
      <el-table :data="competitions" v-loading="loading" stripe>
        ...
        <el-table-column prop="participationType" label="参赛方式" width="100">
          <template #default="scope">
            <el-tag :type="scope.row.participationType === 'TEAM' ? 'warning' : 'success'" effect="plain" size="small">
              {{ scope.row.participationType === 'TEAM' ? '团队赛' : '个人赛' }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" fixed="right" width="240">
          <template #default="scope">
            ...
            <el-button type="warning" link size="small" @click="handleManageTracks(scope.row)">
              管理赛道
            </el-button>
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="参赛方式" prop="participationType">
              <el-select v-model="form.participationType" placeholder="请选择参赛方式">
        ...
        <el-alert
          ...
          :closable="false"
          show-icon
          title="竞赛状态由系统根据报名、提交、评审、公示时间自动计算，无需手动选择。"
    ...
    <!-- 赛道管理对话框 -->
    <TrackManager
      v-model:visible="trackManagerVisible"
      :competition-id="selectedCompetitionId"
      @close="handleTrackManagerClose"
    />
  </div>
</template>
...
import { getCompetitionStatusText, getCompetitionStatusType } from '@/utils/competitionStatus'
...
const trackManagerVisible = ref(false)
...
const handleManageTracks = (row) => {
  selectedCompetitionId.value = row.id
  trackManagerVisible.value = true
}

const handleTrackManagerClose = () => {
  trackManagerVisible.value = false
  selectedCompetitionId.value = null
}

const handleView = (row) => {
  router.push(`/competition/${row.id}?admin=1`)
}
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(
      `确定要删除竞赛"${row.name}"吗？删除后会同步清理该竞赛下的报名、作品、评审、奖项、赛道和团队数据，此操作不可恢复！`,
      '删除竞赛',
      {
