  ...
  <div class="reviews-page">
      ...
      <el-table :data="scores" v-loading="loading" stripe>
        <el-table-column label="参赛对象" width="140">
          <template #default="scope">
            <span>{{ getParticipantDisplayName(scope.row) }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="210" fixed="right">
          <template #default="scope">
            ...
            <el-button type="danger" size="small" plain @click="handleDeleteWork(scope.row)">
    ...
    <el-dialog v-model="detailDialogVisible" title="评审详情" width="600px">
      <el-descriptions :column="1" border v-if="currentDetail">
        ...
        <el-descriptions-item label="参赛对象">{{ getParticipantDisplayName(currentDetail) }}</el-descriptions-item>
...
import { adminDeleteWork } from '@/api/work'
import { downloadWorkFile as downloadFileByUrl } from '@/utils/fileDownload'
...
const downloadWorkFile = async () => {
  ...
  try {
    await downloadFileByUrl(
      currentDetail.value.workFileUrl,
      currentDetail.value.workFileName || currentDetail.value.workTitle || '作品文件'
    )
  } catch (error) {
...
const handleDeleteWork = async (row) => {
  const workTitle = row.workTitle || row.title || '该作品'
  try {
    await ElMessageBox.confirm(
      `确定要删除作品“${workTitle}”吗？删除后会同步清理作品文件、评审任务和获奖记录，此操作不可恢复。`,
      '删除作品',
      {
    ...
    await adminDeleteWork(row.workId)
...
const getParticipantDisplayName = (row) => {
  if (!row) return '-'
  if (row.participationType === 'TEAM') {
    return row.teamName || row.participantName || '-'
  }
