  ...
  <div class="results-page">
        ...
        <el-form-item label="竞赛">
          ...
          >
            ...
            >
              <span>{{ item.name }}</span>
              <el-tag
                :type="getCompetitionStatusType(item.status)"
                effect="plain"
                size="small"
                style="float: right; margin-top: 4px"
              >
                {{ getCompetitionStatusText(item.status) }}
              </el-tag>
            </el-option>
          </el-select>
        </el-form-item>
    ...
    <el-card class="table-card" v-if="selectedCompetition">
      <template #header>
        <div class="card-header">
          <h3>竞赛成绩</h3>
          <div>
            <el-button v-if="canPublishResults" type="primary" size="small" @click="showAwardDialog">
              <el-icon><Trophy /></el-icon>
              <span>发布奖项</span>
            </el-button>
            <el-button v-if="canPublishResults" type="success" size="small" @click="handlePublishAnnouncement">
      ...
      <el-table :data="results" v-loading="loading" stripe>
        ...
        <el-table-column label="参赛对象" width="140">
    ...
    <el-card class="table-card" v-if="selectedCompetition && awards.length > 0" style="margin-top: 24px">
      ...
      <el-table :data="enrichedAwards" stripe>
        ...
        <el-table-column label="获奖对象" width="140">
          <template #default="scope">
            <span>{{ getWinnerName(scope.row) }}</span>
          </template>
        </el-table-column>
        ...
        <el-form-item label="获奖作品" prop="workId">
          <el-select v-model="awardForm.workId" placeholder="请选择作品" style="width: 100%">
            <el-option
              ...
              :key="item.workId"
              :label="`${item.workTitle || '未命名作品'} - ${getWinnerName(item)}`"
              :value="item.workId"
            />
          </el-select>
        </el-form-item>
...
import { getCompetitionStatusText, getCompetitionStatusType, normalizeCompetitionStatus } from '@/utils/competitionStatus'
...
const canPublishResults = computed(() => {
  ...
  const status = normalizeCompetitionStatus(selectedCompetitionInfo.value?.status)
  return (role === 'ADMIN' || role === 'TEACHER') && status === 'REVIEW'
})

const selectedCompetitionInfo = computed(() => {
  return competitionOptions.value.find(item => item.id === selectedCompetition.value)
})
...
const fetchCompetitions = async () => {
  try {
    ...
    competitionOptions.value = (res.data.records || []).filter(item => {
      const status = normalizeCompetitionStatus(item.status)
      return ['REVIEW', 'ANNOUNCEMENT', 'FINISHED'].includes(status)
    })
  } catch (error) {
    console.error(error)
  }
...
const handlePublishAnnouncement = async () => {
  try {
    if (!canPublishResults.value) {
      ElMessage.warning('只有评审中的竞赛可以公示成绩')
      return
    }
