  ...
  <div class="registrations-page">
    ...
    <div class="page-header-wrapper">
      ...
      <div class="page-header">
        <div class="header-content">
          ...
          <p class="page-subtitle">{{ canAudit ? '审核参赛者的报名申请' : '查看与我指导相关的报名记录' }}</p>
        </div>
      </div>
    </div>
    ...
    <el-card class="table-card">
      <el-table :data="registrations" v-loading="loading" stripe>
        ...
        <el-table-column prop="trackName" label="赛道" width="130">
          <template #default="scope">
            <span v-if="scope.row.trackName">{{ scope.row.trackName }}</span>
            <span v-else class="text-muted">-</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" fixed="right" width="220">
          <template #default="scope">
            ...
            <span v-else-if="scope.row.status === 'PENDING'" class="text-muted">待管理员审核</span>
    ...
    <el-dialog v-model="rejectDialogVisible" title="驳回报名" width="500px" @close="rejectReason = ''">
      ...
      <template #footer>
        ...
        <el-button type="danger" @click="confirmReject" :disabled="!canAudit">确认驳回</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import { getRegistrationList, approveRegistration, rejectRegistration, deleteRegistration } from '@/api/registration'
...
const canAudit = computed(() => ['ADMIN', 'TEACHER'].includes(userStore.user?.role))
...
const handleDeleteRegistration = async (row) => {
  const teamTip = row.participationType === 'TEAM'
    ? ' 如果该报名关联的团队没有其他有效报名，团队也会一并删除。'
    : ''
  try {
    await ElMessageBox.confirm(
      `确定删除“${row.projectName || row.participantName || '该报名'}”吗？会同步清理该报名下的作品、评审任务和获奖记录。${teamTip}`,
      '删除报名',
      {
