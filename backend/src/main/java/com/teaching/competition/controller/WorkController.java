...
public class WorkController {
    ...
    public Result<Void> adminDelete(@PathVariable Long id, Authentication authentication) {
        User user = getCurrentUser(authentication);
        if (user == null) {
            return Result.error("用户不存在");
        }
        try {
            workService.deleteWorkAsAdmin(id, user.getId());
            return Result.success();
        } catch (Exception e) {
            return Result.error(500, "删除失败：" + e.getMessage());
        }
    ...
    public Result<PageResult<Work>> getAdminWorks(
        ...
        PageResult<Work> result = workService.getAdminWorks(competitionId, page, size, user.getId());
        return Result.success(result);
    }
}
